...
function run() {
  let focus = ""; // default
  const assert = getJSON("~/Library/DoNotDisturb/DB/Assertions.json").data[0]
  ...
  if (assert) {
    ...
    for (const modeid in config) {
      ...
      if (triggers && triggers.enabledSetting == 2) {
        const start =
          triggers.timePeriodStartTimeHour * 60 +
          triggers.timePeriodStartTimeMinute;
        const end =
          triggers.timePeriodEndTimeHour * 60 +
          triggers.timePeriodEndTimeMinute;
        if (start < end) {
