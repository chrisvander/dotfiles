...
complete -c jj -n "__fish_jj_using_subcommand rebase" -s o -s d -l onto -d 'The revision(s) to rebase onto (can be repeated to create a merge commit)' -r
...
complete -c jj -n "__fish_jj_using_subcommand util; and not __fish_seen_subcommand_from completion config-schema exec gc install-man-pages markdown-help snapshot" -l no-integrate-operation -d 'Run the command as usual but don\'t integrate any operations'
